...
async def quality_flags_from_csv(
    ...
    """
        Эндпоинт для получения всех флагов качества из CSV-файла.
        Включает как базовые флаги, так и новые эвристики из HW03.
        ...
        - id_column: имя колонки для проверки дубликатов ID (опционально)
        """
    start = perf_counter()
    ...
    print(
        ...
        f"zero_ratio_threshold={zero_ratio_threshold} "
        f"id_column={id_column} "
        f"flags_count={len(flags_all)} latency_ms={latency_ms:.1f} ms"
    )

    bool_flags = {k: v for k, v in flags_all.items() if isinstance(v, bool)}
    numeric_flags = {k: v for k, v in flags_all.items() if isinstance(v, (int, float)) and k not in bool_flags}
    other_flags = {k: v for k, v in flags_all.items() if k not in bool_flags and k not in numeric_flags}
